...
// 4. matrix chứa đủ TC + bug
const matrix = fs.readFileSync('tests/traceability-matrix/version-online-with-bugs.md', 'utf8');
for (const id of tcIds) if (!matrix.includes(id)) fail(`matrix thiếu ${id}`);
for (const b of bugFiles) if (!matrix.includes(b)) fail(`matrix thiếu ${b}`);

// 5. spec nào đó phải cover từng TC
...
// 6. evidence index chỉ trỏ file tồn tại (regex khớp cả chữ thường)

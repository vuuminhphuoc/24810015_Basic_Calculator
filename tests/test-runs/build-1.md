# Test Run — Build 1 (bỏ check số)

- SUT: `https://testsheepnz.github.io/BasicCalculator.html`
- Build: `1` (defect bỏ check `isNaN`)
...
| TC-VAL-001 | Validate số ở ô 1 (Abc+9, Add) | ans=``, err=`Number 1 is not a number` | ans=`NaN`, err=`` (không báo lỗi; `tests/test-runs/raw/sweep.json`: Expected `""` Received `"NaN"`) | Fail | BUG-CALC-001 |
| TC-VAL-002 | Validate số ở ô 2 (10+xyz, Add) | ans=``, err=`Number 2 is not a number` | ans=`NaN`, err=`` (không báo lỗi; `tests/test-runs/raw/sweep.json`: Expected `""` Received `"NaN"`) | Fail | BUG-CALC-001 |
...
- `BUG-CALC-001` — Bỏ check số: `Abc+9` ra `NaN` mà không báo lỗi. Phát hiện bởi TC-VAL-001, TC-VAL-002. Evidence: `tests/evidence/B1-badn1.jpg`.

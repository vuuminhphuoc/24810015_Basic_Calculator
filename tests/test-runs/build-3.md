# Test Run — Build 3 (luôn coi là số)

- SUT: `https://testsheepnz.github.io/BasicCalculator.html`
- Build: `3` (defect luôn coi input là số)
...
| TC-CON-001 | Nối chuỗi ab+cd (Concat) | ans=`abcd` | ans=``, err=`Number 1 is not a number` (Concat bị ép sang nhánh số; `tests/test-runs/raw/sweep.json`: Expected `"abcd"` Received `""`) | Fail | BUG-CALC-003 |
...
**Tổng kết: Pass: 13 / Fail: 2 / Tổng: 15.**

## Bugs phát hiện/xác nhận trên build này

- `BUG-CALC-003` — Luôn coi là số: Concat `ab+cd` báo `Number 1 is not a number` thay vì ra `abcd`. Phát hiện bởi TC-CON-001. Evidence: `tests/evidence/B3-concat.jpg`.

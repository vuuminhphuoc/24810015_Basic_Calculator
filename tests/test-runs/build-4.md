# Test Run — Build 4 (khóa integer)

- SUT: `https://testsheepnz.github.io/BasicCalculator.html`
- Build: `4` (defect khóa checkbox integer)
...
| TC-DIV-001 | Chia 10/4 (Div, integer tắt) | ans=`2.5` | ans=`2` (luôn `parseInt`; `tests/test-runs/raw/sweep.json`: Expected `"2.5"` Received `"2"`) | Fail | BUG-CALC-004 |
...
| TC-INT-001 (unchecked) | Chia 5/2, integer tắt | ans=`2.5` | ans=`2` (checkbox bị `disabled`, không bỏ check được; `tests/test-runs/raw/sweep.json`: Expected `"2.5"` Received `"2"`) | Fail | BUG-CALC-004 |
| TC-INT-001 (checked) | Chia 5/2, integer bật | ans=`2` | như Expected (pass vì build luôn làm tròn xuống) | Pass |  |
| TC-CLEAR-001 | Nút Clear xóa kết quả | Clear enabled, sau Clear ans=`` | như Expected | Pass |  |

**Tổng kết: Pass: 12 / Fail: 3 / Tổng: 15.**

## Bugs phát hiện/xác nhận trên build này

- `BUG-CALC-004` — Khóa integer: checkbox bị `disabled` (Prototype: enabled) nên mọi phép chia đều bị `parseInt` (`10/4` ra `2`). Phát hiện bởi TC-DIV-001, TC-INT-001. Evidence: `tests/evidence/B4-div.jpg`.

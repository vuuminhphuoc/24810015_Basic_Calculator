# Test Run — Build 6 (không check chia 0)

- SUT: `https://testsheepnz.github.io/BasicCalculator.html`
- Build: `6` (defect không check chia 0)
...
| TC-DIV-002 | Chia cho 0 (10/0) | ans=``, err=`Divide by zero error!` | ans=`Infinity`, err=`` (không báo lỗi; `tests/test-runs/raw/sweep.json`: Expected `""` Received `"Infinity"`) | Fail | BUG-CALC-006 |
| TC-DIV-003 | Phục hồi UI sau lỗi chia 0 | `calculateButton` + `clearButton` enabled lại | như Expected (Pass vì nhánh `Infinity` có `unlock`, không kẹt nút) | Pass |  |
...
- `BUG-CALC-006` — Không check chia 0: `10/0` ra `Infinity`, không báo `Divide by zero error!`. Phát hiện bởi TC-DIV-002. Evidence: `tests/evidence/B6-div0.jpg`.
